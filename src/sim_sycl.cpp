...
class leenard_jones_kernel;

template<typename T, bool multiple_size, int n_sym>
static inline auto
internal_simulator_on_sycl(sycl::queue &q, size_t work_group_size, const std::span<coordinate<T>> particules, std::span<coordinate<T>> forces, simulation_configuration<T> config, sycl::event evt = {}) {
        ...
        q.submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for<leenard_jones_kernel<T, multiple_size, n_sym>>(
                    ...
                    [=](const sycl::nd_item<1> &item, auto &reducer_x, auto &reducer_y, auto &reducer_z, auto &reducer_energy) {
                        ...
                        /* Whether the current work item takes part in the coputation or not.
                         * We cannot return as it needs to be present for further barriers. */
                        const bool is_active_work_item = [&]() { if constexpr (multiple_size) return true; else { return global_id < particules.size(); }}();

                        /* Setting up local variables */
                        const auto this_work_item_particule = is_active_work_item ? particules[global_id] : coordinate<T>{};
                        static const auto symetries = get_symetries<n_sym, T>();
                        /* Local reducers */
                        ...
                        for (uint32_t tile_id = 0U; tile_id < group_count; ++tile_id) {
                            ...
                            const bool is_active_tile = [&]() { if constexpr (multiple_size) return true; else { return global_id < particules.size(); }}();
                            const uint32_t this_tile_size = [&]() { if constexpr(multiple_size) { return group_size; } else { return std::min<uint32_t>(group_size, particules.size() - tile_id * group_size); }}();
...
#pragma unroll n_sym
                                ...
                                for (const auto &sym: symetries) {
                                    if (global_id == j + tile_id * group_size && sym.x() == 0 && sym.y() == 0 && sym.z() == 0) continue; /* We eliminate the case where the two particles are the same */
...
template<typename T>
std::tuple<coordinate<T>, T> run_simulation_sycl_device_memory(sycl::queue &q, const std::span<coordinate<T>> particules, std::span<coordinate<T>> forces, simulation_configuration<T> config, sycl::event evt) {
...
template std::tuple<coordinate<sycl::half>, sycl::half>
...
template std::tuple<coordinate<float>, float>
...
template std::tuple<coordinate<double>, double>
