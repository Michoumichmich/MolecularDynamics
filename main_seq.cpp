...
template<typename T>
void run_example(const std::vector<coordinate<T>> &coordinates, simulation_configuration<T> config) {
...
int main(int argc, char **argv) {
    ...
    run_example(coordinates, {.use_cutoff = false, .n_symetries = 125, .L_=30});
    ...
    run_example(coordinates_float, {.use_cutoff = false, .n_symetries = 125, .L_=30});
